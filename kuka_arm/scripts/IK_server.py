...
def handle_calculate_IK(req):
	...
	# These matrices will be used to calculate the transformation matrix Rrpy.
	# Rrpy will be the orientation of the EE relative to the base_link.
	R_x = Matrix([[1, 		0, 	  	  0],
		      	  [0, cos(x1), -sin(x1)],
		      	  [0, sin(x1),  cos(x1)]])

	R_y = Matrix([[ cos(x2), 0, sin(x2)],
                  [	      0, 1, 	  0],
                  [-sin(x2), 0, cos(x2)]])

	R_z = Matrix([[cos(x3), -sin(x3), 0],
                  [sin(x3),  cos(x3), 0],
                  [	     0,        0, 1]])

	# Let's try to leave the matrix multiplications outside the for statement. We don't want the code to get even slower!
	Rrpy = R_z * R_y * R_x * R_corr[0:3,0:3]
	R0_3 = T0_1[0:3,0:3] * T1_2[0:3,0:3] * T2_3[0:3,0:3]
        ###

        # Initialize service response
        joint_trajectory_list = []
        #previous_orientation = (0, 0, 0)
        for x in xrange(0, len(req.poses)):
            ...
            #	
            #
            Rrpy = Rrpy.evalf(subs = {x1: roll, x2: pitch, x3: yaw})
            ...
            # If the distance from the requested wrist center 
            # to the second joint is greater than the measurements of the arm, 
            # the position is unreachable.
            if sqrt((wx - j2_x)**2 + (wy - j2_y)**2 + (wz - j2_z)**2) > 2.751:
            	print("Unreachable position for the WC.\n" + str((wx, wy, wz)))
            	return -1
            ...
            # My thetas are given by subtracting the xi angles to pi/2.
